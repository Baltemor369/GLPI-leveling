...
for rang, j in enumerate(joueurs, start=1):
    ...
    with col2:
        st.markdown(f"<span class='{css_rang}' style='font-size:1.1rem'>{j['username']}</span>", unsafe_allow_html=True)
    with col3:
    ...
    with col5:
        # Barre XP compacte
        from sys import path as _p; import os as _os; _p.insert(0, _os.path.join(_os.path.dirname(__file__), "../../sync"))
